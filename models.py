...
def eval_RNN_Model(data_loader, time_step, input_size, model, num_classes, loss_func, name, path):
    ...
    with torch.no_grad():
        ...
        for data_batched, label_batched in data_loader:
            ...
            data_batched = data_batched.reshape(cur_batch_size, time_step,
                                                input_size)  # (batch_size, feature_dim) -> (batch_size, time_step, input_size)
...
class FNN_crelu(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, non_linear='tanh', dropout=0.0):
        ...
        for i in range(self.num_hidden - 1):
            ...
            self.bn.append(nn.BatchNorm1d(hidden_size[i + 1]))
            self.b.append(
                torch.Tensor(np.random.randn(hidden_size[i + 1]) / np.sqrt(hidden_size[i + 1])).to(device=device))
...
class _ComplexLayerNorm(nn.Module):
    ...
    def __init__(self, normalized_shape, eps=1e-5, elementwise_affine=True, bias=True):
        ...
        if self.elementwise_affine:
            self.weight = Parameter(torch.Tensor(self.normalized_shape, 3))
            if bias:
                self.bias = Parameter(torch.Tensor(self.normalized_shape, 2))
            else:
                self.register_parameter('bias', None)
        else:
            self.register_parameter('weight', None)
...
class ComplexLayerNorm1d(_ComplexLayerNorm):
    ...
    def forward(self, input_r, input_i):
        ...
        # calculate mean of real and imaginary part
        mean_r = input_r.mean(dim=-1)
        mean_i = input_i.mean(dim=-1)
        ...
        # Elements of the covariance matrix (biased for train)
        Crr = input_r.var(dim=-1, unbiased=False) + self.eps
        Cii = input_i.var(dim=-1, unbiased=False) + self.eps
        Cri = (input_r.mul(input_i)).mean(dim=-1)
        ...
        input_i = Rri[:, None] * input_r + Rii[:, None] * input_i

        if self.elementwise_affine:
            input_r = self.weight[None, :, 0] * input_r + self.weight[None, :, 2] * input_i
            input_i = self.weight[None, :, 2] * input_r + self.weight[None, :, 1] * input_i
            if self.bias is not None:
                input_r = input_r + self.bias[None, :, 0]
                input_i = input_i + self.bias[None, :, 1]
